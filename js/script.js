...
function checkReveal() {
    reveals.forEach(element => {
        if (element.getBoundingClientRect().top < window.innerHeight - 150) element.classList.add('active');
    });
}
...
async function submitToWeb3Forms(event, successAction) {
    ...
    try {
        ...
        if (result.success) { successAction(); form.reset(); }
...
function closeModal() {
    ...
    document.body.style.overflow = '';
    setTimeout(() => { successDiv.style.display = 'none'; formWrap.style.display = 'block'; }, 400);
}
document.getElementById('tournamentForm').addEventListener('submit', e => submitToWeb3Forms(e, () => {
    formWrap.style.display = 'none'; successDiv.style.display = 'block'; setTimeout(closeModal, 5000);
}));
...
function typeText(elementSelector, text, speed = 80, delay = 0, cursor = true) {
    ...
    setTimeout(() => {
        ...
        const typeInterval = setInterval(() => {
            ...
            else { clearInterval(typeInterval); if (cursor) setTimeout(() => element.classList.remove('typing-cursor'), 1000); }
        }, speed);
    }, delay);
}
function initTypingAnimations() {
    const typingObserver = new IntersectionObserver(entries => entries.forEach(entry => {
        if (entry.isIntersecting && !entry.target.dataset.typed) {
            ...
            typeText('#' + entry.target.id, entry.target.dataset.typeText, parseInt(entry.target.dataset.typeSpeed) || 80, parseInt(entry.target.dataset.typeDelay) || 0, true);
        }
    }), { threshold: 0.5 });
...
// 7. LinkedIn Footer Link
document.addEventListener('DOMContentLoaded', () => {
    const socialLinks = document.querySelector('.social-links');
    if (!socialLinks || socialLinks.querySelector('.sl-linkedin')) return;
    const linkedin = document.createElement('a');
    linkedin.href = 'https://www.linkedin.com/in/alkhassim-lawal-umar-85726942';
    linkedin.target = '_blank';
    linkedin.rel = 'noopener noreferrer';
    linkedin.className = 'social-link sl-linkedin';
