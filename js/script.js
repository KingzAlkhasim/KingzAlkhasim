...
// Portfolio update notice — placed inside the page flow, below the navigation/hero.
document.addEventListener('DOMContentLoaded',()=>{
    ...
    style.textContent='#site-update-notice{width:min(760px,calc(100% - 32px));box-sizing:border-box;margin:28px auto 12px;padding:14px 20px;display:flex;align-items:center;justify-content:center;gap:12px;background:rgba(20,20,28,.88);border:1px solid rgba(255,72,72,.55);border-radius:28px;color:var(--text-secondary);font-size:.82rem;line-height:1.5;text-align:left;box-shadow:0 8px 30px rgba(0,0,0,.16)}#site-update-notice strong{display:block;color:var(--text-primary);font-size:.88rem;margin-bottom:2px}#site-update-notice div span{display:block}.update-icon{width:30px;height:30px;display:grid;place-items:center;flex:0 0 30px;border:1px solid rgba(255,72,72,.45);border-radius:50%;font-size:.85rem;background:rgba(255,72,72,.08)}@media(max-width:600px){#site-update-notice{width:calc(100% - 24px);margin:20px auto 8px;padding:12px 15px;border-radius:24px;font-size:.76rem}.update-icon{width:26px;height:26px;flex-basis:26px}}';
